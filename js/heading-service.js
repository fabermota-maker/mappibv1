...
(function (global) {
  ...
  function createHeadingService(opts = {}) {
    ...
    function readHeading(ev) {
      ...
      const isAbsolute = ev.type === "deviceorientationabsolute" || ev.absolute === true;
      if (isAbsolute) {
    ...
    function handler(ev) {
      ...
      const isAbsoluteEvt = ev.type === "deviceorientationabsolute";
      if (!isAbsoluteEvt && lastAbsoluteAt > Date.now()) return;
      const h = readHeading(ev);
      if (h == null) return;
      if (isAbsoluteEvt) lastAbsoluteAt = Date.now() + 900;
      rawHeading = h;
      targetHeading = h;
      hasSensor = true;
