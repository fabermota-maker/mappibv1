...
(function (global) {
  ...
  const MAX_ROUTE_ALTERNATIVES = 5;
  ...
  function buildRoutePoints(routeEdgeIds, routeNodeIds, edgesById, nodesById) {
    const points = [];
    for (let index = 0; index < routeEdgeIds.length; index++) {
      ...
      const fromNode = nodesById?.get?.(currentNodeId);
      const toNode = nodesById?.get?.(nextNodeId);
      if (fromNode && toNode && edgePoints.length >= 2) {
        ...
        if (reverse + 0.01 < forward) edgePoints = [...edgePoints].reverse();
      }
    ...
    while (open.size) {
      ...
      if (goals.has(current)) {
        ...
        let points = edgeIds.length
          ? buildRoutePoints(edgeIds, nodeIds, graph.edgesById, graph.nodesById)
          : [];
